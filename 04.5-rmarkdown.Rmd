...
It's important to note that when we knit an R Markdown file, it will test our code as if we have an empty environment and it will rely on only the code written in the R Markdown file. It can't use code that was tested in the Console or run interatively in the R Markdown file (more on that soon). 
...
- The three single quotes `"```"` indicates that the lines following until we reach the next three single quotation marks is where some code will be. This is what we call an R chunk. 
- The `{r}` indicates that we are going to write the code using R code.
- Extra information can be added inside the `{r}` notation to give the chunk a name, in this case it is called `setup`.
- The `include = FALSE` means that it will not show up in the rendered report.



This first chunk tells the document how additional chunks should show up in the rendered report by default. Here it says that code should be "echoed" with `echo = TRUE` in the report. You don't need to worry too much about any of this now, just recognize that this is a chunk of code. 
...
This is super helpful for making sure that your code works properly and isn't working simply because you have something in your environment from code that you tested in the console but did not save it anywhere. Other issues can happen if you run a code chunk out of order or changed the code in the chunk after running it previously. Therefore we recommend cleaning the enviornment (which we will describe in the next section) and testing out that your chunks work well together.
...
This will open a menu about how that chunk should be set up. The pulldown menu can be used to select if you want the code to be hidden, the output to be hidden, both, or none to be hidden (the default).
...
If your R Markdown file gets really long, it can be difficult to scroll to find the chunk you want to modify. If you name your chunks, or even if you dont, you can more easily move around from one chunk to another using a special menu button created just for this! 
...
Sometimes we might want to make our reports look a little nicer, perhaps we want to add branding that matches that of our insititute or at least makes the report look really polished. 

You can make changes to the asthetics of the report in very few steps. 

First locate the settings button for the R Markdown editor, which looks like a gear an is located next to the `Knit` button.

```{r, fig.align='center', out.width="100%", echo = FALSE, fig.alt= "The R Markdown settings button is located to the right of the knit button. This opens a menu the helps you change the asthetics of your rendered report."}
...
Then scroll down and select "Output Options". This menu also has nice features if you don't like the defualt ways that the chunks preview output. For example, many people perfer to preview code in the console instead.
...
In summary, R Markdown files can help you to create nice looking reports that help others to understand not only what code you used, but also what the results of your code were. 
 ...
 - New chunks can be added using the new code chunk buttonm which looks like a green square with a "C" in it at the top of the R Markdown editor
