...
Next we want to download and install RStudio. You can do so by going to the Posit website at this link: https://posit.co/. Note that you should be able to download the completely free option. 
...
The Pane Layout menu enables you to change the layout. The image below shows the default settings. 

```{r, fig.align='center', out.width="100%", echo = FALSE, fig.alt= "The Pane Layout menu allows you to modify the layout of RStudio. This shows the default settings with Source on the top left, Console on the bottom left, Environment, History, Connections, and Build on the top right, and Files, Plots, Packages, Help, and VCS on the bottom right."}
...
The Console tab is where we can 'talk' to R and interactively work on our code. The code we write here will not be saved to a script or file, but instead the outcome of the code will immediately be performed we click `enter`.
...
```{r, fig.align='center', out.width="100%", echo = FALSE, fig.alt= "Whe pane on the lower right is where we see our files, plots, and where we get help."}
...
<div class = "dictionary">
- **Console** - The window that allows us to interactively give R code and press enter to run it. 
- **Environment** - R's working memory of objects you have assigned -- told R to remember using `<-`
- **Assignment** - How we tell R to remember something using the `<-` characters. 
</div>
Let's try an example to get started.

As an example, we could type in the code `head(iris)` and press enter to see the code execute and preview.

```{r, fig.align='center', out.width="100%", echo = FALSE, fig.alt= "Running some code in RStudio"}
...
There is a hidden fourth pane. This is only accessible when we start to make a script or a report with our code.
...
In order to make our analysis truly reproducible we will need **every single step** written down. This is why using the Console is great for testing things, but not so great for actually performing your analysis. 
