...
RStudio comes with a nifty feature for organizing your files and making file paths easier for collaborating (more on that in the next section). 

When you create an [R project](https://www.tidyverse.org/blog/2017/12/workflow-vs-script/), which can be made by clicking on the button in the upper left corner of R Studio that looks like a blue cube with the R logo inside of it, you add a `.RProj` file to your working directory.
...
The reason for her anger is that if you write file that starts with your own personal path on your computer, that requires that anyone else who recieves the file to adjust the path for their computer.

The [`here` package](https://here.r-lib.org/) fixes this problem. Instead you can write a path relative to the Rproj file. Then if you send your project files to someone, the paths will work for them too! (This is as long as they don't move the file relative to the other project files.)
